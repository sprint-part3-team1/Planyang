export const createRandomColor = (text: string, light = 80) => {
  ...
  const colorCode = crypto
    .createHash('md5')
    .update(text)
    .digest('hex')
    .substring(0, 6);

  const originR = parseInt(colorCode.substring(0, 2), 16);
  const originG = parseInt(colorCode.substring(2, 4), 16);
  const originB = parseInt(colorCode.substring(4, 6), 16);
