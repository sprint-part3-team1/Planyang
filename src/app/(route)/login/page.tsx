...
const LoginPage = () => {
    ...
    const submitFormData = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if(check) {
            dispatch(loginActions.asynchFetchSignIn({
                email,
                password,
            }));
            ...
            <div className={styles.entireWrapper}>
                ...
                <form className={styles.entireFormWrapper} onSubmit={submitFormData}>
                    <div className={styles.inputFormWrapper}>
                        <Input inputId='email' inputWidth='100%' inputType='text' inputName='이메일' inputRef={emailRef} placeholder='이메일을 입력해 주세요' errorState={emailIsValid} errorMessage='이메일 형식에 맞춰 입력해주세요.' onChange={onChangeEmail}/>
                        <Input inputId='password' inputWidth='100%' inputType='password' inputName='비밀번호' inputRef={passwordRef} placeholder='비밀번호를 입력해 주세요' onChange={onChangePassword} errorMessage='8자 이상 작성해주세요.' errorState={passwordIsValid}/>
                    </div>
                    <LoginButton isActive={loginButtonActive} value='로그인'/>
                </form>
                <div className={styles.signUpWrapper}>
                    <div>회원이 아니시라면?</div>
                    <div className={styles.signUpLink}><Link href='/signup'>회원가입하기</Link></div>
                </div>
            </div>
            <ErrorToast errorMessage={toastMessage} errorState={errorToastState && !successToastState} onClose={() => setErrorToastState(false)} />
            <SuccessToast successMessage='회원가입이 완료되었습니다.' successState={successToastState} onClose={() => {setErrorToastState(false); setSuccessToastState(false)}} />
        </>
    )
}
