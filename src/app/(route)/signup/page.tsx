...
import {isValidCheckPassword, isValidEmail, isValidNickname, isValidPassword} from "@/app/_utils/validateUtils";
...
const SignUp = () => {
    ...
    const formData = useRef({email: '', password: '', nickname: '', checkPassword: ''});
    ...
    const submitFormData = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        const nickname = formData.current.nickname;
        const checkPassword = formData.current.checkPassword;
        let check = true;
        ...
        if(check) {
            dispatch(registerActions.asynchFetchSignUp({
                email,
                nickname,
                password
            }));
        ...
        <>
            <div className={styles.entireWrapper}>
                <div className={styles.logoWrapper}>
                    <Link href='/'><Image src='/assets/images/planyang.png' alt='logo' width={230} height={270}/></Link>
                    <div className={styles.logoTextBox}>첫 방문을 환영합니다!</div>
                </div>
                <form className={styles.entireFormWrapper} onSubmit={submitFormData}>
                    <div className={styles.inputFormWrapper}>
                        <Input inputId='email' inputWidth='100%' inputType='text' inputName='이메일' inputRef={emailRef} placeholder='이메일을 입력해 주세요' errorMessage='이메일 형식에 맞춰 입력해주세요.' onChange={onChangeEmail} errorState={emailIsValid}/>
                        <Input inputId='nickname' inputWidth='100%' inputType='text' inputName='닉네임' inputRef={nicknameRef} placeholder='닉네임을 입력해 주세요' errorMessage='10글자 이하로 입력해주세요.' onChange={onChangeNickname} errorState={nicknameIsValid} />
                        <Input inputId='password' inputWidth='100%' inputType='password' inputName='비밀번호' inputRef={passwordRef} placeholder='비밀번호를 입력해 주세요' errorMessage='8자 이상 작성해주세요.' onChange={onChangePassword} errorState={passwordIsValid}/>
                        <Input inputId='checkPassword' inputWidth='100%' inputType='password' inputName='비밀번호 확인' inputRef={checkPasswordRef} placeholder='비밀번호를 한번 더 입력해 주세요' errorMessage='비밀번호가 일치하지 않습니다.' onChange={onChangeCheckPassword} errorState={checkPasswordIsValid} />
                    </div>
                    <LoginButton isActive={signupButtonActive} value='가입하기'/>
                </form>
                <div className={styles.signUpWrapper}>
                    <div>이미 가입하셨나요?</div>
                    <div className={styles.signUpLink}><Link href='/login'>로그인하기</Link></div>
                </div>
            </div>
            <ErrorToast errorMessage={toastMessage} errorState={errorToastState} onClose={() => setErrorToastState(false)}></ErrorToast>
        </>
    )
}
